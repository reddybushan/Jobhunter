...
/*var str =  sText.replace(/<\/?[^>]+(>|$)/g, "");
var finalStr = str.replace(/^\s+|\s+$/g,"");
	return finalStr;*/
};
...
//Build the menu
var html = widget.oColumn.sTitle; 
var text = $("<div/>").html(html).text();

widget.$Select.empty().append( $( '<option></option>' ).attr( 'value', '' ).text(text) );
aDistinctOptions.sort();
$.each( aDistinctOptions, function( i, sOption ) {
	var sText; 	
	sOption = $("<div/>").html(sOption).text();

sText = $( '<div>' + sOption + '</div>' ).text();
if(null != sText && '' != sText)
   widget.$Select.append( $( '<option></option>' ).attr( {'value': sOption, 'title': sOption }).text( sText ) );
} );
